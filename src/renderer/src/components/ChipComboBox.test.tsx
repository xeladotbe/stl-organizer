...
function setup(
  overrides: Partial<Parameters<typeof ChipComboBox>[0]> = {}
): { onRemove: ReturnType<typeof vi.fn>; onSelectExisting: ReturnType<typeof vi.fn>; onCreateNew: ReturnType<typeof vi.fn> } {
...
describe('ChipComboBox', () => {
  ...
  it('removes the last chip on Backspace when the input is empty', async () => {
    const user = userEvent.setup()
    const { onRemove } = setup({ chips: [{ id: 3, name: 'Vase' }, { id: 4, name: 'Bracket' }] })
